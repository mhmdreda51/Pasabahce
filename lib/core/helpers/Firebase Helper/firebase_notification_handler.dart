...
class FirebaseNotifications {
  ...
  void firebaseCloudMessageListener(BuildContext context) async {
    ...
    _messaging
        .subscribeToTopic("new_notify_nopic")
    ...
    FirebaseMessaging.onMessage.listen((remoteMessage) {
      ...
      if (Platform.isAndroid) {
        if (remoteMessage.data['IMAGE_URL'] != null)
          showNotificationBigStyle(remoteMessage.data['title'],
              remoteMessage.data['body'], remoteMessage.data['IMAGE_URL']);
        else
          showNotification(
  ...
  static void showNotification(title, body) async {
    ...
    var androidChannel = const AndroidNotificationDetails(
        'com.example.flutter_ecom_block_firebase', 'My Channel',
        autoCancel: true,
        ongoing: false,
  ...
  static Future<void> showNotificationBigStyle(title, body, image) async {
    ...
    if (image.toString().contains('http')) {
      bigPicturePath = await _downloadAndSaveFile('${image}', 'bigPicture');
